"""Integration tests for ``GET /api/chat/conversations/{conversation_id}``.
...
def test_get_history_returns_empty_messages_for_freshly_created_session(
    ...
    response = client.get(f"/api/chat/conversations/{conversation_id}", headers=auth_headers)
...
def test_get_history_returns_user_and_assistant_turns_in_order(
    ...
    chat_service._message_store._store[UUID(conversation_id)] = [
...
def test_get_history_returns_404_when_user_does_not_own_session(client: TestClient, two_users: None) -> None:
    ...
    oracle threat-model parity with DELETE /api/chat/conversation/{id}.
    """
    del two_users
    ...
    chat_service._message_store._store[UUID(alice_conversation_id)] = [
        ModelRequest(parts=[UserPromptPart(content="alice's secret trip plan")]),
    ]

    # Bob must NOT see alice's history.
    bob_response = client.get(f"/api/chat/conversations/{alice_conversation_id}", headers=bob_headers)
    assert bob_response.status_code == 404

    # Alice still can.
    alice_response = client.get(f"/api/chat/conversations/{alice_conversation_id}", headers=alice_headers)
