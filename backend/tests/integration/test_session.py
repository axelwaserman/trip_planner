...
class TestDeleteSession:
    def test_delete_removes_session_from_all_dicts(self, client: TestClient) -> None:
    ...
    def test_delete_already_deleted_session_returns_404(self, client: TestClient) -> None:
        session_response = client.post("/api/chat/session")
        session_id = session_response.json()["session_id"]
...
class TestChatInvalidSession:
    def test_invalid_session_streams_empty_error_event(self, client: TestClient) -> None:
