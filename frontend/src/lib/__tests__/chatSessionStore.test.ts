...
describe('chatSessionStore', () => {
  ...
  it('writes update the snapshot for the targeted session only', () => {
    setSession('a', () => ({ messages: [{ role: 'user', content: 'hi' }], isAwaitingFirstChunk: true, isStreaming: true }))
  ...
  it('notifies subscribers when any session updates', () => {
    ...
    setSession('a', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: true }))
    expect(listener).toHaveBeenCalledTimes(1)

    setSession('b', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: false }))
    expect(listener).toHaveBeenCalledTimes(2)

    unsubscribe()
    setSession('c', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: false }))
    expect(listener).toHaveBeenCalledTimes(2)
  })

  it('getStreamingSessionIds reflects only sessions with isStreaming=true', () => {
    setSession('streaming-a', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: true }))
    setSession('streaming-b', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: true }))
    setSession('idle-c', () => ({ messages: [], isAwaitingFirstChunk: false, isStreaming: false }))
